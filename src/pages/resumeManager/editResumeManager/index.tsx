...
import { isRequiredFieldsFilled } from "./helpers"
...
const EditResumeManager = () => {
  ...
  const handleEditExperience = (index) => {
    ...
    setEditExperienceData({ ...experienceToEdit, _id: index })
  ...
  const handleSaveExperience = (id, updatedExperience) => {
    setExperiences((prevExperiences) =>
      prevExperiences.map((exp, idx) => (idx === id ? updatedExperience : exp))
    )
  ...
  const handleUpdateResume = async () => {
    ...
    const resumeId = resumeResponse?._id
    ...
    if (
      ...
      errorToast("Missing resume data from local storage")
      return
    }
