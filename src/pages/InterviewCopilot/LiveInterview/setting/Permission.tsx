...
const Permission = () => {
  const mandatoryList = [
    ...
    },
    {
      ...
      warning:
        "You need to install the Final Round AI Google extension to receive program advice from our Coding Copilot. Add from Chrome Web Store and enable it.",
    }
  ]
    ...
    <div className="flex flex-col gap-4 ">
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4">
          {mandatoryList.map((mandatory, index) => (
            <MandatoryPanel
              ...
              requestUrl={mandatory.requestUrl}
            />
          ))}
        </div>
        ...
        <MandatoryPanel
            ...
            content={"Get timely updates on interview report progress and special offers."}
            requestUrl={"https://"}
            warning={null}
        />

        <p className="flex items-center text-lg gap-2">I don't Know How to enable permissions <span><a href="" target="_blank"><img src={openBrowserIcon} alt="" /></a></span></p>
...
const MandatoryPanel = ({ src, title, content, warning, requestUrl }) => {
  return (
    <div className="flex items-start gap-4 bg-[#2c2c2c] p-4 rounded-lg">
      <div className="flex flex-col gap-4 w-full text-white">
        ...
        <div className="flex justify-between items-center">
          ...
          {requestUrl && (
            <Button className="text-white bg-[#404040] px-4 py-2 rounded-md text-sm mt-2 hover:underline">
              Request
            </Button>
          )}
        </div>
        {warning && <p className="text-lg text-red-500 bg-[#5e1f1f] mt-2 p-4 rounded-lg">{warning}</p>}
      </div>
    </div>
  )
}
